...
/**
 * Adaptar o código fonte fornecido, para receber por argumentos de linha de comandos o ficheiro com o polígono
 * em análise, o numero de processos filho e o número total de pontos aleatórios a considerar.
 * ex: ./monteCarlo poligon.txt 3 900
 * @param filename nome do ficheiro a analisar
 * @param num_processes numero de processos
 * @param num_points numero de pontos aleatórios a considerar
 */
void reqA(char *filename, int num_processes, int num_points) {

    int points = num_points/num_processes;
    ...
    printf("Os pontos foram adicionados!\n");
}

void reqB(char *filename, int num_processes, int num_points, Point polygon[], int n) {
    ...
    for (int i = 0; i < num_processes; i++) {
        ...
        if (pid < 0) {
            ...
            fprintf(file, "Process id: %d\nPoints generated by child: %d\nPoints inside polygon:%d\n", getpid(),
                    num_points, points_polygon);
            fclose(file);
            exit(0);
        }
    }
...
void reqC(char *filename, int num_processes, int num_points, Point polygon[], int n) {
    ...
    for (int i = 0; i < num_processes; i++) {
        ...
        if (pid < 0) {
            ...
            fprintf(file, "Process id: %d\nPoints generated by child: %d\nPoints inside polygon:%d\n", getpid(),
                    num_points, points_polygon);
            fclose(file);
            write(pipes[i][1], &points_polygon, sizeof(int));
            close(pipes[i][1]);
            exit(0);
        }
    }
    int total_points_polygon = 0;
    for (int i = 0; i < num_processes; i++) {

        int points_polygon;
        read(pipes[i][0], &points_polygon, sizeof(int));
        total_points_polygon += points_polygon;
        close(pipes[i][0]);
    }

    double area = (double)total_points_polygon / num_points * 4.0;
...
int main(int argc, char *argv[]) {
    ...
    reqA(filename, num_processes, num_points);
    //reqB(filename, num_processes, num_points, polygon, 371);
    //reqC(filename, num_processes, num_points, polygon, 200);
    reqD(num_processes);

    // usado nos requesitos
    /*int pointsInside = 0;
